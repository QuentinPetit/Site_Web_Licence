			...
			count($_POST['matiereSelect'])>0) 
		{
			...
			if ($fileExtension == "zip") {
				if (is_dir("../Projets/".$anneeDossier."/".$dossierParcours."/".$fileName) == false) {
					...
					$messageZip = "Votre fichier .zip a bien été pris en compte.";
				} else {
					$errorZip =  $fileName." a déjà été mis en ligne";
				}
			} else {
				$errorZip = "Désolé, seuls les fichiers .zip sont pris en compte";
			}
			...
			$sql="INSERT INTO projets (ID_Annee, Nom, Date, Description, Caracteristique, Logiciel, Materiel, Poids, Miniature, Fichier_Projet, Lien) VALUES ('".$anneeScolaireSelect."', '".utf8_decode($nomProjets)."', NOW(), '".utf8_decode($description)."', '".utf8_decode($caracteristiques)."', '".utf8_decode($software)."', '".utf8_decode($hardware)."', '".$poids."', '".$miniature."', '".$projectFile."', '".$site."');";
			$conn->query($sql);
			...
			foreach ($dataPath as $singleData) {
				...
				if (isset($type)) {
					...
					$errorFileType = "Ce type de fichier n'est pas pris en compte";
				}
			}
			...
			$messageupload = "Votre projet a été mis en ligne correctement";
		} else {
			$errorupload = "Tous les champs nécessaires n'ont pas étés remplis";
		}
		...
		<div class="col-xs-12 col-md-8 col-md-offset-2">
			<form action="ajoutProjet.php" method="POST" enctype="multipart/form-data">
				<h1>Ajouter un projet</h1>
					...
					if (isset($errorZip))
					{
						echo "<div class='alert alert-danger'>".$errorZip."</div>"; 
					}
					...
					if (isset($messageZip)) 
					{
						echo "<div class='alert alert-success'>".$messageZip."</div>";
					}
					if (isset($messageupload)) 
					{
						echo "<div class='alert alert-success'>".$messageupload."</div>";
					}
					?>
